...
outCSV = "./LossCSV/Losstest.csv"  # 将损失写入csv
...
def resultStr(str1, str2, str3, str4, str5, str6, str7):
    liststr = []
...
def fit_one_epoch(net, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            if dice_loss:
                ...
                loss2 = Tversky_loss(outputs, labels, alpha=2, beta=2, sigma=0)
                loss = loss1 + loss2
            ...
            pbar.set_postfix(**{'total_loss': total_loss / (iteration + 1),
                                'Focal_loss': total_loss1 / (iteration + 1),
                                'Tversky_loss': total_loss2 / (iteration + 1),
    ...
    with tqdm(total=epoch_size_val, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                if dice_loss:
                    # val_loss2 = Dice_loss(outputs, labels)
            ...
            pbar.set_postfix(**{'val_total_loss': val_total_loss / (iteration + 1),
                                ...
                                'val_Tversky_loss' : val_total_loss2 / (iteration + 1),
    ...
    print(
        'Train Total Loss: %.4f||Train Focal_Loss: %.4f||Train Tversky_loss: %.4f ||Val Loss: %.4f||Val Focal_loss: %.4f||Val Tversky_loss: %.4f' % (
            total_loss / (epoch_size + 1), total_loss1 / (epoch_size + 1), total_loss2 / (epoch_size + 1), val_total_loss / (epoch_size_val + 1), val_total_loss1 / (epoch_size_val + 1), val_total_loss2 / (epoch_size_val + 1)))
    ...
    write2File(
        resultStr(epoch, total_loss / (epoch_size + 1), total_loss1 / (epoch_size + 1), total_loss2 / (epoch_size + 1),
                  val_total_loss / (epoch_size_val + 1), val_total_loss1 / (epoch_size_val + 1), val_total_loss2 / (epoch_size_val + 1)))


if __name__ == "__main__":
    time1 = time.time()
    fileHead = "Epoch,Train_total_loss,Train_FL_loss,Train_Tversky_loss,Val_total_loss,Val_FL_loss,Val_Tversky_loss"
